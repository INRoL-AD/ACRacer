...
try:
    ...
    ac.log("[DataParser] Error importing libraries: %s" % e)
...
def get_local_velocity():
    """
    Retrieve the current local velocity of a ego vehicle in float array
    :return: current velocity [x,y,z]
    """
    return info.physics.localVelocity

def get_local_velocity_pythonAPI(car: int = 0):
    """
    Retrieve the current local velocity of a ego vehicle in float array
    :return: current velocity [x,y,z]
    """
    return ac.getCarState(car, acsys.CS.LocalVelocity)

def get_local_angular_velocity():
    """
    Retrieve the current local angular velocity of a ego vehicle in float array
    :return: current angular velocity [x,y,z]
    """
    return info.physics.localAngularVel

def get_local_angular_velocity_pythonAPI(car: int = 0):
    """
    Retrieve the current local velocity of a ego vehicle in float array
    :return: current velocity [x,y,z]
    """
    return ac.getCarState(car, acsys.CS.LocalAngularVelocity)
...
def get_world_orientation():
    """
    Retrieve absolute roll/pitch/yaw of a car
    :return: absolute heading [roll, pitch, yaw] in float array
    """
    yaw = info.physics.heading
    pitch = info.physics.pitch
    roll = info.physics.roll
    return [roll, pitch, yaw]

def get_camber_angle(car: int = 0):
    """
    Retrieve slip angle in degree
    :return: slip angle [0,360]
    """
...
def get_slip_ratio(car: int = 0):
    """
    Retrieve slip ratio
    :return: slip ratio list [FL, FR, RL, RR]
    """
    return ac.getCarState(car, acsys.CS.SlipRatio)

def get_tyre_load(car: int = 0):
    """
    Retrieve slip ratio
    :return: slip ratio list [FL, FR, RL, RR]
    """
...
def get_velocity_pythonAPI(car: int = 0):

    return ac.getCarState(car, acsys.CS.Velocity)
